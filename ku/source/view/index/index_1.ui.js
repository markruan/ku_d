...
function getdata() {
	...
	http.url = "http://v7idc.com/ku/index.php?r=home/api/getgedan&type=1&page="
			+ ye; // 请求的 URL
...
function gethuainfo() {
	...
	http.timeout = 30000; // 超时时间 : 单位 毫秒
	http.contentType = "application/json"; // Content-Type
	http.url = "http://v7idc.com/ku/index.php?r=home/api/tuijian"; // 请求的 URL
	// http.body = JSON.stringify({id:type_id, page:pageNum}); // 传入新闻类型ID和页码的参数
	http.on("success", function(data) {
		if (data != "null") {
...
function getmv() {
	var mvlistui = ui("do_GridView_2")

	var http = mm("do_Http");
	http.method = "GET"; // GET | POST
	http.timeout = 30000; // 超时时间 : 单位 毫秒
	http.contentType = "application/json"; // Content-Type
	http.url = "http://v7idc.com/ku/admin.php?r=home/api/mvlist"; // 请求的 URL
	// http.body = JSON.stringify({id:type_id, page:pageNum}); // 传入新闻类型ID和页码的参数
	http.on("success", function(data) {
		if (data.code==200) {

			mvlistdata.removeAll()
			mvlistdata.addData(data)
			mvlistui.bindItems(mvlistdata)
		}
	});
	http.on("fail", function(data) {
